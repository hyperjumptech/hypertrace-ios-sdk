...
class CentralController: NSObject {
  ...
  func turnOn() {
    ...
    central = CBCentralManager(delegate: self, queue: queue, options: [CBCentralManagerOptionRestoreIdentifierKey: restoreIdentifierKey])
  }

  func turnOff() {
    Logger.DLog("CC turnOff")
    guard let theCentral = central else {
      return
    }
    theCentral.stopScan()
    central = nil
...
extension CentralController: CBCentralManagerDelegate {
  ...
  func centralManagerDidUpdateState(_ central: CBCentralManager) {
    ...
    case .poweredOn:
      Logger.DLog("Central Powered ON")
