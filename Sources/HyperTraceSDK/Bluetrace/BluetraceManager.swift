...
class BluetraceManager {
  ...
  func presentBluetoothAlert(_ bluetoothStateString: String) {
    #if DEBUG
      let alert = UIAlertController(title: "Bluetooth Issue: " + bluetoothStateString + " on " + Device.current.description + " iOS: " + UIDevice.current.systemVersion, message: "Please screenshot this message and send to support!", preferredStyle: .alert)
  ...
  func getCentralState() -> CBManagerState {
    return centralController.getState() ?? .unknown
  }
