...
class V2Peripheral: PeripheralProtocol {
  ...
  func prepareReadRequestData(onComplete: @escaping (Data?) -> Void) {
    if advtPayloadExpiry == nil || Date() > advtPayloadExpiry! {
      EncounterMessageManager.shared.fetchBatchTempIds { (error: Error?, resp: (tempIds: [TempId], refreshDate: Date)?) in
...
class V2Central: CentralProtocol {
  func prepareWriteRequestData(tempId: String, rssi: Double, txPower _: Double?) -> Data? {
    do {
      let dataToWrite = CentralWriteDataV2(
        mc: Device.current.description,
        rs: rssi,
        id: tempId,
