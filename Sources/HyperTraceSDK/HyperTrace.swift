...
public class HyperTrace {
  ...
  public var requirements: HyperTraceRequirements = .init()
  ...
  public func start(baseUrl: String, uid: String = "", sessionConfiguration: URLSessionConfiguration? = nil) {
    _ = API.shared(baseUrl: baseUrl, sessionConfiguration: sessionConfiguration)
    setIdentity(uid)
    DatabaseManager.shared().persistentContainer = persistentContainer
  ...
  public func upload(code: String, onComplete: ((Error?) -> Void)?) {
    API.shared().getUploadToken(code: code) { error, token in
      ...
      self.getUploadData { getUploadDataError, encounters in
        ...
        API.shared().uploadData(token: uploadToken, traces: traces) { uploadError, _ in
          if uploadError != nil {
...
public extension HyperTrace {
  /// Remove encounters from local database.
  /// - Parameters:
  ///   - olderThan: The value of cut off date.
  ///   - unit: The unit of time.
  static func removeData(olderThan: Int = BluetraceConfig.TTLDays, unit: Calendar.Component = .day) {
  ...
  static func countEncounters(olderThan: Int = BluetraceConfig.TTLDays, unit: Calendar.Component = .day) -> Int {
  ...
  ///   - unit: The unit of the time.
  /// - Returns: The number of encounters.
  static func countEncounters(inTheLast: Int = BluetraceConfig.TTLDays, unit: Calendar.Component = .day) -> Int {
...
public extension HyperTrace {
  ...
  static func setScanningInterval(_ interval: Int) {
  ...
  static func setScanningDuration(_ duration: Int) {
